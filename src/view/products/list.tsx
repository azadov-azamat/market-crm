...
export default function ProductList() {
    ...
    return (
        <div className={"product-list w-full h-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5"}>
            {
                productList.map((item, ind) =>
                    <ProductBox text={item.name} img={item.img} id={ind} price={item.price} measure={item.measure} count={item.count}/>
                )
            }
        </div>
    );
